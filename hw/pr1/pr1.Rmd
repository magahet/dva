...
parse_awards <- function(msg) {
  awards = 0
  nominations = 0
  for (pattern in c('won (\\d+)', "(\\d+) win")) {
    m = str_match(msg, pattern)[1, 2]
    if (!is.na(m)) {
      awards = awards + as.integer(m)
    }
  }
  for (pattern in c('nominated for (\\d+)', "(\\d+) nomination")) {
    m = str_match(msg, pattern)[1, 2]
    if (!is.na(m)) {
      nominations = nominations + as.integer(m)
    }
  }
  return(c(awards, nominations))
}

a = sapply(df$Awards, parse_awards)
